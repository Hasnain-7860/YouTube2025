import React from 'react'
import { Link } from 'react-router-dom';
import song2 from '../assets/song2.jpg'
import thumbnail8 from '../assets/thumbnail8.png'
import thumbnail1 from '../assets/thumbnail1.png'
import thumbnail2 from '../assets/thumbnail2.png'
// import thumbnail8 from '../assets/thumbnail8.png'
import before from '../assets/before.webp'
import thumbnail4 from '../assets/thumbnail4.png'
import thumbnail5 from '../assets/thumbnail5.png'
import thumbnail6 from '../assets/thumbnail6.png'
import thumbnail7 from '../assets/thumbnail7.png'
import salman1 from '../assets/salman1.webp'
import singer from '../assets/singer.jpg'
import salman from '../assets/salman.png'
import codeharry from '../assets/codeharry.webp'
import PlayingVideo from './PlayingVideo';













const Video = () => {
  
  return (
    <div className='mt-20 grid grid-cols-1 md:grid-cols-2 gap-14 lg:grid-cols-4 '>
       
       <Link to={<Video/>}><div className=''>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    </Link>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    <div className='card'>
        <img className='rounded-md' src={thumbnail2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={before} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail4} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail6} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail7} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={salman1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
   
    <div className='card'>
        <img className='rounded-md' src={salman} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
  
    <div className='card'>
        <img className='rounded-md' src={singer} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={codeharry} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    <div className=''>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    <div className='card'>
        <img className='rounded-md' src={thumbnail2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={before} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail4} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail6} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail7} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={salman1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
   
    <div className='card'>
        <img className='rounded-md' src={salman} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
  
    <div className='card'>
        <img className='rounded-md' src={singer} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={codeharry} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    <div className=''>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    <div className='card'>
        <img className='rounded-md' src={thumbnail2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={before} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail4} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail6} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail7} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={salman1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
   
    <div className='card'>
        <img className='rounded-md' src={salman} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
  
    <div className='card'>
        <img className='rounded-md' src={singer} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={codeharry} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    <div className=''>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    <div className='card'>
        <img className='rounded-md' src={thumbnail2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={before} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail4} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail6} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail7} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={salman1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
   
    <div className='card'>
        <img className='rounded-md' src={salman} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
  
    <div className='card'>
        <img className='rounded-md' src={singer} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={codeharry} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    <div className=''>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    <div className='card'>
        <img className='rounded-md' src={thumbnail2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={before} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail4} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail6} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail7} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={salman1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
   
    <div className='card'>
        <img className='rounded-md' src={salman} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
  
    <div className='card'>
        <img className='rounded-md' src={singer} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={codeharry} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    <div className=''>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    <div className='card'>
        <img className='rounded-md' src={thumbnail2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={before} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail4} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail6} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail7} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={salman1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
   
    <div className='card'>
        <img className='rounded-md' src={salman} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
  
    <div className='card'>
        <img className='rounded-md' src={singer} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail5} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={codeharry} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={song2} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail8} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div>
    <div className='card'>
        <img className='rounded-md' src={thumbnail1} alt="" /> 
        <h2>Best channel to learn coding that help you to be a web devloper</h2>
        <h3>Greatstack</h3>
        <p>15k views & bull; 2 days ago</p>
    </div> 
    </div>
  )
}

export default Video